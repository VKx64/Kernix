...
export function TasksTriagePage() {
  ...
  const create = async (values: CreateTaskPayload, files: File[] = []) => {
    ...
    setCreateError('')
    try {
      // One task, however many people are on it: `assignee_user_ids` is the
      // shape the API takes, and it writes the whole set in one transaction.
      // An absent list leaves the owner to the server.
